...
while read -r -a columns; do
	...
	# ssUx,y,z, Bx,y,z resid & iters
	elif [[ ${columns[solver_label_col]} = $smoothSolver_label ]] || [[ ${columns[solver_label_col]} = $DICPCG_label ]]; then
		...
		elif [[ ${columns[solver_var_label_col]} = $DICPCG_p_label ]]; then
			if [[ $pRepeats = 1 ]]; then
